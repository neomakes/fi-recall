...
소방관은 가장 훈련을 많이 하는 직업 중 하나지만, **현장에서 배운 교훈은 다음 훈련에 거의 반영되지 않습니다.** 사후검토(AAR)는 의무지만 "교훈 포착 → 교리 갱신" 사이가 끊깁니다 — IAFC조차 정식 보고서가 *"수년 걸리거나, 아예 안 써진다"*고 적습니다.

![IAFC Hotwash Form](hotwash-form.png)
*IAFC "Hotwash" 양식 — 교훈을 빨리 잡으려는 시도 자체가, 루프가 끊겼다는 증거.*

> *지휘관 훈련을 받은 장교로서, 교훈이 다음 교리로 이어지지 못한 채 사라지는 걸 직접 봤습니다.* — 창업자
...
**① 교리 지도화 [훅]** — 텍스트 SOP를 한눈에 보이는 **교리 지도**(절차·판단점·위험 노드 + 관계)로 구조화. 압박 속 빠른 이해.

**② 복기 반영 [킥]** — 피드백을 넣으면 닿는 노드를 태깅·재발 표시 후 격차별로 닫습니다:
- **교리 격차**(*아는 것*) → **SOP 수정 제안**
- **실행 격차**(*몸*) → **재훈련 drill**

**구현 핵심:** `SOP 한 섹션 → 교리 지도 → 피드백 1건 → 태깅 + (수정안/drill)`. *(교리 지도 = SOP 온톨로지)*
